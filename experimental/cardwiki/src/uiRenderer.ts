module UiRenderer {
  ...
  export class UiRenderer {
    public compiled = 0;
    protected tagCompilers:{[tag: string]: (elem:microReact.Element) => void} = {};

    compile(roots:(string|microReact.Element)[]):microReact.Element[] {
      let compiledElems:microReact.Element[] = [];
    ...
    protected _compileWrapper(template:string, baseIx: number, boundAliases?:string[], bindingStack:any[] = []):microReact.Element[] {
      let elems = [];
    ...
    protected _compileElement(template:string, bindingStack:any[], fact?:any):microReact.Element {
      ...
      let elem:microReact.Element = {t: base["ui template: tag"], ix: base["ui template: ix"]};
