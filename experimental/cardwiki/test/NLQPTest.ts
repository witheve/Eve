...
function executeQuery(query: nlqp.Query): Array<string> {
  ...
  if (query.projects.length !== 0) {
    ...
    window['DEBUG'].instrumentQuery = true;
    queryString = `
    (query
       
      (select "entity eavs" :entity "AUTOGENERATED koala THIS SHOULDN'T SHOW UP ANYWHERE" :attribute "length" :value koala|length)
      (select "entity eavs" :entity pet :attribute "length" :value pet|length)
      (select "is a attributes" :entity pet :collection "AUTOGENERATED pet THIS SHOULDN'T SHOW UP ANYWHERE")
      (negate 
        (select ">" :a pet|length :b koala|length)
      )
	    (project! :pet pet :koala "AUTOGENERATED koala THIS SHOULDN'T SHOW UP ANYWHERE")
    )
    `;
