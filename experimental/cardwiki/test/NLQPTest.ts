    ...
    results.forEach((result) => {
      ...
      let rows: Array<Array<string>> = projected.map((row) => {
        let rowstring = keys.map((key,i) => {
          ...
          let filtered = `${row[key]}`.replace("AUTOGENERATED","");
          filtered = filtered.replace("THIS SHOULDN'T SHOW UP ANYWHERE","");
          // Get the width of each row
          if (colWidths[i] < filtered.length) {
            colWidths[i] = filtered.length;
          }
          return filtered;
        });
        return rowstring;
